...
items_id_cat_platform = pd.read_csv('engineered_datasets/items_id_cat_platform.csv')
...
class good_shops_items0(BaseEstimator, TransformerMixin) : 
    ...
    def fit(self, X, y) : 
        ...
        def shop_open(df) :
            ...
            cnt_shop_first_open = cnt_shop_first_open['first_open_month_num'].astype(int)
    ...
    def transform(self, X) : 
        ...
        X_ = X_.reset_index()
        X_ = pd.merge(X_, self.shop_open, how = 'left', on = 'shop_id').set_index('index')
...
def one_hot_fit_transform(df) : 
    ...
    OH_cols = pd.DataFrame(OH_encoder.transform(df_object), columns = OH_encoder.get_feature_names(onehot_cols1)) 
