...
# create 4-month rolling means for lag_cols = ['item_cnt_month', 'item_price_mean']

# we choose a time-series split of train and validation sets, and the latter to span the last three months (Aug - Oct 2014)
...
for col in lag_cols : 
    ...
    for n in range(lag_months) : 
        train_df_month_test = pp.rolling_mean(train_df_month_test, col, n + 1)

# train_df_month_test0 = train_df_month_test.copy()


#%% train-validation split

# we do a time series split; training set contains Apr 2013 to Jul 2015, validation set contains Aug 2015 - Oct 2015

df_train = train_df_month_test[(train_df_month_test['date_block_num'] > lag_months) & (train_df_month_test['date_block_num'] < 34 - lag_months )]

df_valid = train_df_month_test[(train_df_month_test['date_block_num'] > 34 - lag_months - 1) & (train_df_month_test['date_block_num'] < 34 )]
...
feature_cols = ['month', 'year', 'shop_city', 'shop_type', 'item_main_category',
       'platform', 'first_open_month_num'] + [col + '_lag_' + str(i+1) for i in range(lag_months) for col in lag_cols] + [col + '_rolling_' + str(i+1) for i in range(lag_months) for col in lag_cols]
...
my_pred = pd.Series(RFR.predict(encoded_X_test_scaled).clip(0,20), name = 'item_cnt_month', index = test_df_all['ID'])
# test score 1.69278
my_pred.to_csv('my_submission_RFR.csv')
...
RFR_model_rmse = RandomizedSearchCV(RFR_model, param_distributions = random_grid_RFR, scoring = 'neg_root_mean_squared_error', cv = None, verbose = 1, n_iter = 10)
