...
# create 4-month lag features for lag_cols = ['item_cnt_month', 'item_price_mean'], and <<12-month lag feature>> for 'item_cnt_month'. This choice is informed by their auto-correlation functions plots

# we choose a time-ordered split of train and validation sets, and the latter to span (Feb - Oct 2015)
...
train_df_month = train_df_month[ 
    ...
    & (train_df_month['item_price'] > 0.1) 
    & (train_df_month['item_price'] < 100000 # about HKD10,000; a PS5 is HKD6,000
       )]

# train_df_raw.shape # (2935849, 6)

#%% drop 'date' column, add 'month' and 'year' columns
...
train_df_month_test = train_df_month_test.drop(['item_cnt_month_lag_' + str(i) for i in np.arange(5, 12)], axis = 1).drop(['item_price_mean_lag_' + str(i) for i in np.arange(5, 13)], axis = 1)

#%% train-validation split

# we do a time series split; training set contains Jan 2014 to Jan 2015, validation set contains Feb 2015 - Oct 2015

df_train = train_df_month_test[(train_df_month_test['date_block_num'] > lag_months) & (train_df_month_test['date_block_num'] < 26 )]

df_valid = train_df_month_test[(train_df_month_test['date_block_num'] > 25) & (train_df_month_test['date_block_num'] < 34 )]
...
#        'platform', 'first_open_month_num'] + [col + '_lag_' + str(i+1) for i in range(lag_months) for col in lag_cols]
feature_cols = ['month', 'year', 'shop_city', 'shop_type', 'item_main_category',
       'platform', 'first_open_month_num'] + ['item_cnt_month_lag_' + str(i) for i in [1,2,3,4,12]] + ['item_price_mean_lag_' + str(i) for i in [1,2,3,4]]
...
# item_cnt_month_lag_12         0.130546
# item_cnt_month_lag_4          0.110192
# item_cnt_month_lag_3          0.100924
# item_cnt_month_lag_2          0.096364
# item_cnt_month_lag_1          0.092163
# shop_city_encoded             0.083414
# month                         0.081757
# item_main_category_encoded    0.059740
# item_price_mean_lag_1         0.048444
# item_price_mean_lag_3         0.048272
# item_price_mean_lag_2         0.048111
# item_price_mean_lag_4         0.047821
# shop_type_encoded             0.040621
# year                          0.008557
# first_open_month_num          0.003075
# dtype: float64
...
RFR_model_rmse = RandomizedSearchCV(RFR_model, param_distributions = random_grid_RFR, scoring = 'neg_root_mean_squared_error', cv = 2, verbose = 1, n_iter = 20)
...
# item_cnt_month_lag_4          0.159563
# item_cnt_month_lag_12         0.159231
# item_cnt_month_lag_3          0.158793
# item_cnt_month_lag_1          0.147439
# item_cnt_month_lag_2          0.142664
# item_main_category_encoded    0.059661
# month                         0.036318
# item_price_mean_lag_4         0.025907
# item_price_mean_lag_3         0.025673
# item_price_mean_lag_2         0.024684
# item_price_mean_lag_1         0.024054
# shop_city_encoded             0.019733
# year                          0.007721
# shop_type_encoded             0.007366
# first_open_month_num          0.001194
# dtype: float64
...
# test score 1.63816
my_pred.to_csv('my_submission_RFR_best_3.csv')
