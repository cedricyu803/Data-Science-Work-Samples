...
shop_id_city_type = pd.read_csv('engineered_datasets/shop_id_city_type.csv', index_col = [0])
items_id_cat_platform = pd.read_csv('engineered_datasets/items_id_cat_platform.csv', index_col = [0])
...
def plots4():
    ...
    train_df_raw[['item_cnt_day']].hist(bins = 100, grid = False, ax = plt.gca(), color = 'skyblue')
    plt.yscale('log')
    ax = plt.gca()
    ...
    fig = plt.figure(dpi = 150)
    train_df_raw[['item_price']].hist(bins = 100, grid = False, ax = plt.gca(), color = 'seagreen')
    plt.yscale('log')
    ax = plt.gca()
...
def train_cnt_shop1(df0):
    df = df0.pivot_table(values = 'item_cnt_day', index = 'shop_id', columns = 'date_block_num', aggfunc = [np.nansum])
...
train_cnt_shop_still_open = train_cnt_shop_still_open['open_last_month'].astype(int)
...
train_cnt_shop_first_open = train_cnt_shop_first_open['first_open_month_num'].astype(int)


train_cnt_shop_open = pd.merge(train_cnt_shop_still_open, train_cnt_shop_first_open, how = 'inner', left_index = True, right_index = True)
...
for i in train_cnt_shop.index : 
    ...
    plot_acf(train_cnt_shop.loc[i], ax = plt.subplot(8,8,j), label = str(i), color = plot_color)
    j = j + 1
    ax = plt.gca()
    ax.set_xlabel(None)
    ax.set_title(None)
...
for i in train_cnt_shop.index : 
    ...
    plot_pacf(train_cnt_shop.loc[i], ax = plt.subplot(8,8,j), label = str(i), color = plot_color)
    j = j + 1
    ax = plt.gca()
    ax.set_xlabel(None)
    ax.set_title(None)
...
train_cnt_shop_city_type = pd.merge(shop_id_city_type, train_cnt_shop, how = 'left', left_index = True, right_index = True)

train_cnt_shop_city = train_cnt_shop_city_type.drop(['shop_type', 'shop_id'], axis = 1).groupby(['shop_city']).agg(np.nansum)
train_cnt_shop_type = train_cnt_shop_city_type.drop(['shop_city', 'shop_id'], axis = 1).groupby(['shop_type']).agg(np.nansum)
...
def train_cnt_shop_month_num1(df0):
    df = df0.pivot_table(values = 'item_cnt_day', index = 'shop_id', columns = 'month_num', aggfunc = [np.nansum])
    df.fillna(0, inplace = True)
    df.reset_index(inplace = True)
    df.columns = df.columns.get_level_values(1)
    df.rename(columns = {'': 'shop_id'}, inplace = True)
...
def train_cnt_shop_month_year1(df0):
    df = df0.pivot_table(values = 'item_cnt_day', index = 'shop_id', columns = ['month_num', 'year'], aggfunc = [np.nansum])
    df.fillna(0, inplace = True)
    df.reset_index(inplace = True)
    df.columns = zip(df.columns.get_level_values(1), df.columns.get_level_values(2))
    df.rename(columns = {('', ''): 'shop_id'}, inplace = True)
    df.set_index('shop_id', inplace = True)
    return df
...
train_cnt_shop_month_2013 = train_cnt_shop_month_year_all_shops.loc[[(j, 1) for j in np.arange(1,13)]]
train_cnt_shop_month_2013.rename(index = dict(zip(train_cnt_shop_month_2013.index, np.arange(1, 13))), inplace = True)
train_cnt_shop_month_2014 = train_cnt_shop_month_year_all_shops.loc[[(j, 2) for j in np.arange(1,13)]]
train_cnt_shop_month_2014.rename(index = dict(zip(train_cnt_shop_month_2014.index, np.arange(1, 13))), inplace = True)
train_cnt_shop_month_2015 = train_cnt_shop_month_year_all_shops.loc[[(j, 3) for j in np.arange(1,11)]]
train_cnt_shop_month_2015.rename(index = dict(zip(train_cnt_shop_month_2015.index, np.arange(1, 11))), inplace = True)

fig = plt.figure(dpi=150)
pd.concat([train_cnt_shop_month_2013, train_cnt_shop_month_2014, train_cnt_shop_month_2015], axis = 1).rename(columns = {0: 2013, 1: 2014, 2: 2015}).plot(kind = 'bar', xlabel = 'Month', ax = plt.gca(), rot=0)
ax = plt.gca()
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
def train_cnt_item1(df0) : 
    df = train_df.pivot_table(values = 'item_cnt_day', index = 'item_id', columns = 'date_block_num', aggfunc = [np.nansum])
    df.fillna(0, inplace = True)
    df.reset_index(inplace = True)
    df.columns = df.columns.get_level_values(1)
...
train_cnt_item_cat_platform = pd.merge(items_id_cat_platform, train_cnt_item, how = 'left', on = 'item_id')

""" group by category"""
train_cnt_item_cat = train_cnt_item_cat_platform.drop(['item_category_id', 'item_id', 'platform'], axis = 1).groupby(['item_main_category']).agg(np.nansum)
...
""" group by platform"""
train_cnt_item_pltform = train_cnt_item_cat_platform.drop(['item_id', 'item_main_category', 'item_category_id'], axis = 1).groupby(['platform']).agg(np.nansum)
# scale sales by the mean
train_cnt_item_pltform_scaled = train_cnt_item_pltform.apply(scale_row_mean, axis = 1)
...
train_cnt_item_still_sold = train_cnt_item.apply(sold_last_three_months, axis = 1)
train_cnt_item_still_sold = train_cnt_item_still_sold['sold_last_three_months'].astype(int)
...
train_cnt_item_first_sold = train_cnt_item_first_sold['first_sold_month_num'].astype(int)

train_cnt_item_sold = pd.merge(train_cnt_item_still_sold, train_cnt_item_first_sold, how = 'inner', left_index = True, right_index = True)
...
# make pivot table
train_df_still_sold = pd.merge(train_df, train_cnt_item_sold, how = 'left', on = 'item_id')
train_df_still_sold = train_df_still_sold[train_df_still_sold['sold_last_three_months'] == 1]

train_item_price = train_df_still_sold.pivot_table(values = 'item_price', index = 'item_id', columns = 'date_block_num', aggfunc = [np.nanmean])
...
for i in np.arange(0, 300) : 
    train_item_price_scaled.iloc[i,].plot(xlabel = 'Month since 2013-01', ax = plt.gca())
ax = plt.gca()
# ax.set_xlabel(None)
# ax.set_title(None)
ax.spines['top'].set_visible(False)
...
train_price_item_cat_platform = pd.merge(items_id_cat_platform, train_item_price, how = 'left', on = 'item_id')

""" group by category"""
train_price_item_cat = train_price_item_cat_platform.drop(['item_category_id', 'item_id', 'platform'], axis = 1).groupby(['item_main_category']).agg(np.nanmean)
# scale prices by the means
train_price_item_cat_scaled = train_price_item_cat.apply(scale_row_mean, axis = 1)
...
"""group by platform"""
train_price_item_pltform = train_price_item_cat_platform.drop(['item_id', 'item_main_category', 'item_category_id'], axis = 1).groupby(['platform']).agg(np.nanmean)
# scale prices by the means
train_price_item_pltform_scaled = train_price_item_pltform.apply(scale_row_mean, axis = 1)
...
for i in train_price_item_cat.index : 
    plot_acf(train_price_item_cat.loc[i], ax = plt.subplot(3,5,j), label = str(i))
    plt.legend()
    j = j + 1
    ax = plt.gca()
    ax.set_xlabel(None)
...
j = 1 
for i in train_price_item_cat.index : 
    plot_pacf(train_price_item_cat.loc[i], ax = plt.subplot(3,5,j), label = str(i))
    plt.legend()
    j = j + 1
    ax = plt.gca()
    ax.set_xlabel(None)
