...
XGBR_model = XGBRegressor(eval_metric="rmse",
                          ...
                          reg_lambda=0.9,
                          n_jobs=6)
XGBR_model.fit(X_train_encoded4_scaled, y_train, eval_set=[
               (X_train_encoded4_scaled, y_train), (X_valid_encoded4_scaled, y_valid)], early_stopping_rounds=50)
...
XGBR_feature_importances = pd.Series(
    XGBR_model.feature_importances_, index=original_cols).sort_values(ascending=False)
...
sns.barplot(y=XGBR_feature_importances.iloc[:18].index,
            x=XGBR_feature_importances.iloc[:18].values, color='skyblue')
...
grid_values_XGBR = {'n_estimators': list(np.arange(100, 1100, 100)), 'learning_rate': [
...
XGBR_model_mse = RandomizedSearchCV(XGBR_model, param_distributions=grid_values_XGBR,
                                    scoring='neg_mean_squared_error', cv=None, verbose=1, n_iter=10)
XGBR_model_mse.fit(X_train_encoded4_scaled, y_train, **fit_params)
print('Grid best parameter (min. neg_root_mean_squared_error): ',
      XGBR_model_mse.best_params_)


mean_squared_error(y_train, XGBR_model_mse.predict(
    X_train_encoded4_scaled), squared=False)
mean_squared_error(y_valid, XGBR_model_mse.predict(
...
RFRfeature_imp = pd.Series(RFR.feature_importances_,
                           index=X_train_encoded4_scaled.columns).sort_values(ascending=False)
...
fig = plt.figure('feature_importances', figsize=(10, 6), dpi=200)
sns.barplot(y=RFRfeature_imp.iloc[:18].index,
            x=RFRfeature_imp.iloc[:18].values, color='skyblue')
# LGBMreg_feature_importances.plot.barh()
ax = plt.gca()
ax.set_xlabel('Feature importance')
ax.set_ylabel(None)
...
RFRfeature_imp_best_old = pd.read_csv(
    'old/RFR_old_best_feat.csv', usecols=[0, 2], header=None, index_col=[0]).squeeze()

fig = plt.figure('feature_importances', figsize=(10, 6), dpi=200)
sns.barplot(y=RFRfeature_imp_best_old.index,
            x=RFRfeature_imp_best_old.values, color='skyblue')
# LGBMreg_feature_importances.plot.barh()
ax = plt.gca()
ax.set_xlabel('Feature importance')
ax.set_ylabel(None)
...
RFR_model_rmse = RandomizedSearchCV(RFR_model, param_distributions=random_grid_RFR,
                                    scoring='neg_mean_squared_error', cv=None, verbose=1, n_iter=6)
RFR_model_rmse.fit(X_train_encoded4_scaled, y_train)
print('Grid best parameter (min. neg_root_mean_squared_error): ',
      RFR_model_rmse.best_params_)
...
RFR_best = RandomForestRegressor(n_estimators=650, max_depth=60, max_features='sqrt',
                                 min_samples_leaf=4, min_samples_split=128, random_state=0, n_jobs=-1)
RFR_best.fit(X_train_encoded4_scaled, y_train)

print(mean_squared_error(y_train, RFR_best.predict(
    X_train_encoded4_scaled), squared=False))
print(mean_squared_error(y_valid, RFR_best.predict(
    X_valid_encoded4_scaled), squared=False))
# 2.074137686378509
# 1.8754997033673564

pd.Series(RFR_best.feature_importances_,
          index=X_train_encoded4_scaled.columns).sort_values(ascending=False)
...
LGBMreg_feature_importances = pd.Series(
    LGBMreg.feature_importances_, index=original_cols).sort_values(ascending=False)
...
LGBMreg_search = RandomizedSearchCV(LGBMreg2, param_distributions=grid_values_LGBM,
                                    scoring='neg_mean_squared_error', cv=None, verbose=1, n_iter=10)
