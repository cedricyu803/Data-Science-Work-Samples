...
shop_id_city_type = pd.read_csv('engineered_datasets/shop_id_city_type.csv')
...
items_id_cat_platform = pd.read_csv(
    'engineered_datasets/items_id_cat_platform.csv')
items_id_cat_platform['platform'].fillna('other', inplace=True)
items_id_cat_platform = items_id_cat_platform[[
    'item_id', 'item_category_id', 'item_main_category', 'platform']]

# %% merge shop duplicate pairs from fuzzy_match_pair
...
def lags(df, col, n=3):
    ...
    for i in np.arange(1, n + 1):
        ...
        df0 = pd.merge(df0, df_lag, how='left', on=[
...
class good_shops_items0(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        def shop_open(df):
            cnt_shop = df.pivot_table(
                values='item_cnt_month', index='shop_id', columns='date_block_num', aggfunc=[np.nansum])
            ...
            def open_last_month(row):
                """ change definition here if needed"""
            ...
            cnt_shop_still_open = cnt_shop_still_open['open_last_month'].astype(
                int)

            def first_open_month(row):
                row['first_open_month_num'] = (
                    row[row > 0].index[0] - row.index[0]) / (row.index[-1] - row.index[0])
                return row

            cnt_shop_first_open = cnt_shop.apply(first_open_month, axis=1)
            cnt_shop_first_open = cnt_shop_first_open['first_open_month_num'].astype(
                int)

            cnt_shop_open = pd.merge(
                cnt_shop_still_open, cnt_shop_first_open, how='inner', left_index=True, right_index=True)

            return cnt_shop_open

        self.shop_open = shop_open(df_).astype(int)

        def item_sold(df):

            cnt_item = df.pivot_table(
                values='item_cnt_month', index='item_id', columns='date_block_num', aggfunc=[np.nansum])
            ...
            cnt_item_still_sold = cnt_item.apply(
                sold_last_three_months, axis=1)
            cnt_item_still_sold = cnt_item_still_sold['sold_last_three_months'].astype(
                int)

            def first_sold_month(row):
                if len(row[row > 0]) > 0:
                    row['first_sold_month_num'] = (
                        row[row > 0].index[0] - row.index[0]) / (row.index[-1] - row.index[0])
                else:
            ...
            cnt_item_first_sold = cnt_item_first_sold['first_sold_month_num'].astype(
                int)

            cnt_item_sold = pd.merge(
                cnt_item_still_sold, cnt_item_first_sold, how='inner', left_index=True, right_index=True)
        ...
        """ only keep shops still open and items sold recently"""
        df_ = df_[(df_['open_last_month'] == 1) & (
            df_['sold_last_three_months'] == 1)]
        df_.drop(['open_last_month', 'sold_last_three_months',
                 'first_sold_month_num'], axis=1, inplace=True)
    ...
    def transform(self, X):
        ...
        X_ = X_.reset_index()
        X_ = pd.merge(X_, self.shop_open, how='left',
                      on='shop_id').set_index('index')
        ...
        X_['first_open_month_num'] = X_['first_open_month_num'].astype(int)
...
def one_hot_fit_transform(df):
    ...
    df_object = df[onehot_cols1]
    OH_encoder.fit(df_object)
    OH_cols = pd.DataFrame(OH_encoder.transform(
        df_object), columns=OH_encoder.get_feature_names(onehot_cols1))
    OH_cols.index = df.index
    num_X = df.drop(onehot_cols1, axis=1)
