...
class HyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        num_dense_hidden = hp.Int(
            'num_dense_hidden', min_value=1, max_value=6, step=1)
        ...
        if num_dense_hidden > 1:
            for l in np.arange(1, num_dense_hidden):
                units = hp.Int('units' + str(l), min_value=32,
...
encoded_X_train = pd.read_csv(
    'encoded_X_train.csv').drop(['Unnamed: 0'], axis=1)
encoded_X_train_scaled = pd.read_csv(
    'encoded_X_train_scaled.csv').drop(['Unnamed: 0'], axis=1)
y_train = pd.read_csv('y_train.csv').drop(['Unnamed: 0'], axis=1)
encoded_X_valid = pd.read_csv(
    'encoded_X_valid.csv').drop(['Unnamed: 0'], axis=1)
encoded_X_valid_scaled = pd.read_csv(
    'encoded_X_valid_scaled.csv').drop(['Unnamed: 0'], axis=1)
...
encoded_X_test_scaled = pd.read_csv(
    'encoded_X_test_scaled.csv').drop(['Unnamed: 0'], axis=1)

my_pred0 = my_model.predict(encoded_X_test_scaled).clip(0, 20)

my_pred = pd.Series(my_pred0.squeeze(),
                    name='item_cnt_month', index=test_df_all['ID'])
# test score 1.69278
